        ...
        spotifyApi.getPlaylist(data.rows[0].spotify_id)
          .then((data) => {
            data.body.tracks.items.forEach(song => {
              ...
              songResult.push({ image: song.track.album.images[0].url, name: song.track.name, album: song.track.album.name, releaseDate: song.track.album.release_date, duration: duration, playCount: Math.floor(Math.random() * 400) })
            })
